 ...
 * - Public search discovery without fake authenticated sessions
...
export class CrawleeNationaleHulpgidsScraper {
   ...
   * Returns false until a verified authenticated flow is implemented.
   */
  async authenticate(): Promise<boolean> {
    console.warn(
      "[Crawlee] Nationale Hulpgids authenticated automation is not implemented; using public search only"
    );
    return false;
  }
  ...
  async searchCandidates(criteria: {
    ...
    console.log(
      "[Crawlee] Starting candidate search with criteria:",
    ...
    const crawler = new CheerioCrawler(
      {
        ...
        async requestHandler({ request, $, log }) {
          ...
          $(".candidate-card, .helper-card, .profile-item").each(
            (index, element) => {
              const $el = $(element);
              ...
              const candidate: ScrapedCandidate = {
                ...
                email: $el
                  .find(".email, [href^='mailto:']")
                  .first()
                  .attr("href")
                  ?.replace("mailto:", ""),
                phone: $el
                  .find(".phone, .tel, [href^='tel:']")
                  .first()
                  .text()
                  .trim(),
                location:
                  $el
                    .find(".location, .address, .city")
                    .first()
                    .text()
                    .trim() || criteria.location,
                distance:
                  parseFloat($el.find(".distance").first().text()) || undefined,
                experience: $el
                ...
                rating:
                  parseFloat($el.find(".rating, .stars").first().text()) ||
                  undefined,
              ...
              if (
                ...
                candidate.distance &&
                candidate.distance > criteria.maxDistance
              ) {
  ...
  async getCandidateProfile(
    ...
    console.log(`[Crawlee] Fetching profile: ${describeScraperTarget(profileUrl)}`);
    ...
    const crawler = new CheerioCrawler(
      {
        ...
        async requestHandler({ request, $, log }) {
          log.info(`Processing profile ${describeScraperTarget(request.url)}`);
 ...
 */
export function createCrawleeScraper(
  config: CrawleeScraperConfig
): CrawleeNationaleHulpgidsScraper {
